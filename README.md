...
- **On-node bulk buttons**: directly under the search bar sit **✓ all** and **✗ none** (tick/untick every row in one click — with an active on-node search or the **☑ only** filter they act only on the visible rows and show the match count, e.g. **✓ all 12**), **☑ only** (view filter: show only the ticked rows; combines with the search, click again to show all rows) and the **⇅ use** sort button (lights up while the frequency order is active, click again to restore the stored order).
