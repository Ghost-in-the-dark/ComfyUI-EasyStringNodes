...
- **On-node bulk buttons**: directly under the search bar sit **✓ all** and **✗ none** (tick/untick every row in one click — with an active on-node search they act only on the matched rows and show the match count, e.g. **✓ all 12**) and the **⇅ by use** sort button (the old header ⇅ moved here as a readable labelled button; it lights up while the frequency order is active, click again to restore the original order).
- **Usage ranking**: every row keeps a hidden `freq` counter that the node bumps once each time the row is actually selected by a run (`select_all`, `line_numbers`, preset or ticked checkbox). Used rows show a warm tint plus a **×N** counter on the right; the **⇅ by use** button sorts the list by frequency (most-used first; click again to restore the original order), and the dialog has a **⇅ by use** button too. Counters persist in the workflow/dataset, so the ranking survives reloads.
